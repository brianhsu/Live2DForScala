...
object CardanoAlgorithm {
  val Epsilon = 0.00001f

  def forBezier(a: Float, b: Float, c: Float, d: Float): Float = {
   ...
   */
  private def limitToRange(value: Float, min: Float, max: Float): Float = value.max(min).min(max)

  private def quadraticEquation(a: Float, b: Float, c: Float): Float = {
    if (Math.abs(a) < Epsilon) {
