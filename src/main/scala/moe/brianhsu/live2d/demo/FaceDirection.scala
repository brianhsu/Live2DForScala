...
object FaceDirection {
  ...
  def update(deltaTimeSeconds: Float): Unit = {
    ...
    val MaxA = deltaTimeWeight * MaxV / FrameToMaxSpeed       // 1frameあたりの加速度

    // 目指す向きは、(dx, dy)方向のベクトルとなる
    val dx = _faceTargetX - _faceX
    val dy = _faceTargetY - _faceY
    ...
    // ※本来、人間は筋力で力（加速度）を調整できるため、より自由度が高いが、簡単な処理ですませている
    {
      ...
      val maxV: Float = 0.5f * (sqrt((MaxA * MaxA) + 16.0f * MaxA * d - 8.0f * MaxA * d) - MaxA).toFloat
